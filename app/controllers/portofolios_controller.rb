class PortofoliosController < ApplicationController
    ...
    layout('portofolio')
     access all: [:show, :index, :angular], user: {except: [:destroy, :new, :create, :edit, :destroy, :update, :sort]}, site_admin: :all
 ...
 private
       ...
       def set_blog 
        ...
        def portofolio_params
            params.require(:portofolio).permit(:title, :subtitle, :body, :main_image, :thumb_image, technologies_attributes: [:name])
        end
